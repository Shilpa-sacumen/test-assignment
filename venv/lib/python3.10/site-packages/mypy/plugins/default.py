...
def typed_dict_setdefault_callback(ctx: MethodContext) -> Type:
    ...
    if (
        ...
        if assigned_readonly_keys:
            ctx.api.msg.readonly_keys_mutated(
                assigned_readonly_keys, context=ctx.context
            )
...
_TP_DICT_MUTATING_METHODS: Final = frozenset(
    {"update of TypedDict", "__ior__ of TypedDict"}
)
    ...
    if isinstance(ctx.type, TypedDictType) and len(signature.arg_types) == 1:
        ...
        if ctx.args and ctx.args[0]:
            ...
            with ctx.api.msg.filter_errors(
                filter_errors=lambda name, info: info.code
                != codes.TYPEDDICT_READONLY_MUTATED,
