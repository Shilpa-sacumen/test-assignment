...
class _Encoder(Protocol):
    ...
    ) -> tuple[bytes, int]: ...  # signature of Codec().encode

class _Decoder(Protocol):
    def __call__(
        self, input: ReadableBuffer, errors: str = ..., /
    ) -> tuple[str, int]: ...  # signature of Codec().decode

class _StreamReader(Protocol):
    def __call__(
        self, stream: _ReadableStream, errors: str = ..., /
    ) -> StreamReader: ...

class _StreamWriter(Protocol):
    def __call__(
        self, stream: _WritableStream, errors: str = ..., /
    ) -> StreamWriter: ...
...
def iterencode(
    iterator: Iterable[str], encoding: str, errors: str = "strict"
) -> Generator[bytes, None, None]: ...
def iterdecode(
    iterator: Iterable[bytes], encoding: str, errors: str = "strict"
...
BOM_UTF32: Final[
    Literal[b"\xff\xfe\x00\x00", b"\x00\x00\xfe\xff"]
]  # depends on `sys.byteorder`
...
class StreamWriter(Codec):
    ...
    def __getattr__(
        self, name: str, getattr: Callable[[Any, str], Any] = ...
    ) -> Any: ...
...
class StreamReader(Codec):
    ...
    def readlines(
        self, sizehint: int | None = None, keepends: bool = True
    ) -> list[str]: ...
    def reset(self) -> None: ...
    def __enter__(self) -> Self: ...
    def __exit__(
