...
class TestIntegration(base.BaseTestCase):
    ...
    @testtools.skipUnless(
        os.environ.get("PBR_INTEGRATION", None) == "1", "integration tests not enabled"
    )
    def test_integration(self):
        ...
        venv = self.useFixture(
            test_packaging.Venv(
                "sdist", modules=["pip", "wheel", PBRVERSION], pip_cmd=PIP_CMD
            )
        )
        python = venv.python
        self.useFixture(
            base.CapturedSubprocess("sdist", [python, "setup.py", "sdist"], cwd=path)
        )
        venv = self.useFixture(
            test_packaging.Venv(
                "tarball", modules=["pip", "wheel", PBRVERSION], pip_cmd=PIP_CMD
            )
        )
        python = venv.python
        ...
        venv = self.useFixture(
            test_packaging.Venv(
                "install-git", modules=["pip", "wheel", PBRVERSION], pip_cmd=PIP_CMD
            )
        )
        root = venv.path
        python = venv.python
        ...
        venv = self.useFixture(
            test_packaging.Venv(
                "install-e", modules=["pip", "wheel", PBRVERSION], pip_cmd=PIP_CMD
            )
        )
        root = venv.path
        python = venv.python
        self.useFixture(
            base.CapturedSubprocess("install-e", [python] + pip_cmd + ["-e", path])
        )
...
class TestMarkersPip(base.BaseTestCase):
    ...
    def test_pip_versions(self):
        ...
        self._run_cmd(
            bin_python,
            ["-m", "pip", "install", "--no-index", "-f", repo_dir, "test_markers"],
            cwd=venv.path,
            allow_fail=False,
        )
        pkgs = self._run_cmd(
            bin_python, ["-m", "pip", "freeze"], cwd=venv.path, allow_fail=False
        )[0]
...
class TestLTSSupport(base.BaseTestCase):
    ...
    def test_lts_venv_default_versions(self):
        ...
        self._run_cmd(
            bin_python, ["-m", "pip", "install", pbr], cwd=venv.path, allow_fail=False
        )
